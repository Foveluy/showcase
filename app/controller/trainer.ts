...
for (let i = 9; i < 23; i++) {
  const keyLen = Object.keys(timeIndex).length
  timeIndex[`${i}:00`] = keyLen
  timeIndex[`${i}:30`] = keyLen + 1
}
...
export default class Trainer extends Base {
  ...
  public async bookTrainer() {
    ...
    for (let i in TodayBooked) {
      ...
      if (timeIndex[item.start] > timeIndex[end] || timeIndex[item.end] < timeIndex[start]) {
        booked = false
      } else {
        //如果被预定立刻跳出
        booked = true
